...
class __login__:
    ...
    def __init__(self, auth_token: str, company_name: str, width, height, logout_button_name: str = 'Logout', hide_menu_bool: bool = False, hide_footer_bool: bool = False):
        """
        Arguments:
        -----------
        1. self
    ...
    def build_login_ui(_self):
        ...
        auth_json_exists_bool = _self.check_auth_json_file_exists('_secret_auth_.json')

        if auth_json_exists_bool == False:
            with open("_secret_auth_.json", "w") as auth_json:
                json.dump([], auth_json)

        main_page_sidebar, selected_option = _self.nav_sidebar()
        ...
        if _self.hide_menu_bool == True:
            _self.hide_menu()
        
        if _self.hide_footer_bool == True:
            _self.hide_footer()
